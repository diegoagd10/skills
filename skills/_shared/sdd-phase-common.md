...
3. If neither was provided, scan the installed skills directory for `*/SKILL.md`, read each frontmatter (`name`, triggers/`description`), match the triggers to your task, and read the matching `SKILL.md` paths.
4. If no skill matches, proceed with your phase skill only.

NOTE: the preferred path is (1) — exact skill paths selected by the orchestrator. Paths (2) and (3) are fallbacks. Scanning the skills directory is SKILL LOADING, not delegation. If `## Skills to load before work` is present, IGNORE redundant `SKILL: Load` instructions.
...
- `skill_resolution`: how skills were loaded — `paths-injected` (received exact skill paths from orchestrator), `fallback-scan` (self-loaded paths by scanning the skills directory), `fallback-path` (loaded via SKILL: Load path), or `none` (no skills loaded)
...
(other values: `fallback-scan`, `fallback-path`, or `none — no skills found`)
```
...
- The orchestrator MUST default the SDD delivery strategy to `single-pr` and pass that canonical value to `sdd-tasks` and `sdd-apply`.
- `sdd-tasks` MUST forecast whether the planned work may exceed that budget.
- The forecast MUST include exact plain-text guard lines: `Decision needed before apply: Yes|No`, `400-line budget risk: Low|Medium|High`, and `Maintainer-approved size exception: Yes|No`.
- If the forecast is high, `sdd-tasks` MUST report the risk and keep the delivery model single-PR.
- `sdd-apply` MUST NOT start oversized work unless the run explicitly carries maintainer-approved `size:exception`.
- Any PR boundary language must stay single-PR focused; do not recommend chained or stacked PR slices.
